...
static osTimerId_t beep_timer_handle  = NULL;

// 内部函数声明
static void offline_task(void *argument);
...
void offline_init(void)
{
    ...
    const osThreadAttr_t offlineTask_attributes = {
      .name = "offlineTask",
      .stack_size = 256 * 4,
      .priority = (osPriority_t) osPriorityAboveNormal7,
    };
    offline_task_handle = osThreadNew(offline_task, NULL, &offlineTask_attributes);
    ...
    // 创建10ms周期的定时器
    beep_timer_handle = osTimerNew(test_timer_callback, 
                                 osTimerPeriodic, 
                                 NULL, 
                                 &myTimer01_attributes);
    
    // 启动定时器，使用osKernelGetTickFreq()确保准确的时间
    if (beep_timer_handle != NULL) {
        osStatus_t status = osTimerStart(beep_timer_handle,   pdMS_TO_TICKS(10)); 
        if (status != osOK) {
